...
const BUCKET = "campaign-bestiary-images";
...
export async function POST(req: NextRequest, context: RouteContext) {
    try {
        ...
        if (!file.type.startsWith("image/")) {
            return NextResponse.json({ error: "El archivo debe ser una imagen valida." }, { status: 400 });
        }
        if (file.size > MAX_IMAGE_BYTES) {
            return NextResponse.json({ error: "La imagen supera el limite de 50 MB." }, { status: 400 });
        }
        ...
        const entryRow: BestiaryStorageRow | null = basicRow
            ? { id: basicRow.id, image_storage_bucket: null, image_storage_path: null }
            : null;
        ...
        const oldBucket = entryRow.image_storage_bucket ?? BUCKET;
        const oldPath = entryRow.image_storage_path ?? null;

        if (oldPath) {
            const { error: removeError } = await adminClient.storage.from(oldBucket).remove([oldPath]);
        ...
        const message =
            error instanceof Error ? error.message : "Error subiendo imagen de la criatura.";
